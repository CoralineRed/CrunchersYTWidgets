...
import {Group, Text} from '@jetbrains/ring-ui';
import {Color} from '@jetbrains/ring-ui/components/icon/icon__constants';
...
class IssueLine extends React.Component {
  ...
  render() {
    ...
    const ErrorInPercentsFieldName = 'Error %';
    const errAndErrInPercFields = valuableFields.filter(x => x.projectCustomField.field.name === ErrorFieldName || x.projectCustomField.field.name === ErrorInPercentsFieldName).map(x => {
      return {label: x.projectCustomField.field.name, value: x.value};
    });
    ...
    return (
      ...
      >
        ...
        <Group>
          {
            errAndErrInPercFields.map(x => {
              return <Text
                key={x.label}
                style={x.label === ErrorInPercentsFieldName && x.value > 0 ? {color: Color.RED} : x.label === ErrorInPercentsFieldName ? {color: Color.GREEN} : {}}>
                {x.label}: {x.value}
              </Text>;
            })
          }
